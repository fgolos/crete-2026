...
Every push to `main` automatically deploys to GitHub Pages, and the deployment workflow can also be started manually. The workflow builds an explicit `_site` directory containing only runtime assets and audio files. Pages deployments share one concurrency group, and an active deployment is allowed to finish before a later one starts.
